...
final class AssistantClient {
 ...
 private final Context activity;private final Callback callback;
 ...
 AssistantClient(Context a,Callback c){activity=a;callback=c;}
 ...
 void ask(String id,String payload){if(id==null||!id.matches("[a-zA-Z0-9-]{1,80}"))return;if(payload==null||payload.length()>65000){callback.result(id,"","The conversation is too large. Start a new conversation.");return;}if(!configured()){callback.result(id,"","Connect AI first.");return;}if(!busy.compareAndSet(false,true)){callback.result(id,"","The assistant is still answering. Please wait.");return;}
 worker.execute(()->{HttpsURLConnection con=null;try{JSONObject incoming=new JSONObject(payload);JSONArray input=incoming.getJSONArray("input");if(input.length()>9)throw new IOException();JSONObject request=new JSONObject().put("model",MODEL).put("store",false).put("max_output_tokens",3000).put("instructions",incoming.getString("instructions")).put("input",input).put("text",incoming.getJSONObject("text"));

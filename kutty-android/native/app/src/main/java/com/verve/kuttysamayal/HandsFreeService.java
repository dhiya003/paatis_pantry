...
public class HandsFreeService extends Service {
 ...
 private volatile boolean ended=false,ready=false,ttsReady=false;private volatile String phase="Starting",requestId="",lastReply="",lastError="";private volatile long deadline,lastWake=0;private int turn=0;
 private final Runnable expire=()->finishSession("60-minute session finished");
 private final Runnable commandTimeout=()->{if(!ended&&(phase.equals("Listening")||phase.equals("Opening speech recognition"))){lastError="No speech arrived. Try Ask now, or check your phone speech service.";cancelSpeech();idle();}};
 ...
 static String status(){HandsFreeService s=instance;JSONObject o=new JSONObject();try{o.put("active",s!=null&&!s.ended);o.put("remainingSeconds",s==null?0:Math.max(0,(s.deadline-SystemClock.elapsedRealtime())/1000));o.put("phase",s==null?lastReason:s.phase);o.put("lastReply",s==null?"":s.lastReply);o.put("lastError",s==null?"":s.lastError);o.put("lastWake",s==null?0:s.lastWake);o.put("micReady",s!=null&&s.wake!=null&&s.wake.capturing&&SystemClock.elapsedRealtime()-s.wake.lastAudio<2000);o.put("micLevel",s==null||s.wake==null?0:s.wake.level);o.put("micSilenced",s!=null&&s.wake!=null&&s.wake.silenced);}catch(Exception ignored){}return o.toString();}
 ...
 @Override public int onStartCommand(Intent intent,int flags,int id){
  ...
  try{channel();if(Build.VERSION.SDK_INT>=29)startForeground(72,notification("Starting hands-free"),ServiceInfo.FOREGROUND_SERVICE_TYPE_MICROPHONE);else startForeground(72,notification("Starting hands-free"));
   ...
   wake=new WakeListener(this,new WakeListener.Callback(){public void ready(){if(live()&&phase.equals("Starting microphone"))phase("Say Hey Kutty");}public void keyword(String word){onKeyword(word);}public void error(){finishSession("Microphone unavailable — open Kutty to try again");}});
 ...
 void onKeyword(String word){if(!live())return;String normalized=word.toLowerCase(Locale.ROOT).replace('_',' ').trim();if(normalized.equals("stop")){finishSession("Hands-free stopped");return;}if(phase.equals("Say Hey Kutty")&&ready&&normalized.equals("hey kutty")){lastWake=System.currentTimeMillis();beginCommand();}}
 void listenNow(){if(live()&&ready&&(phase.equals("Say Hey Kutty")||phase.equals("Starting microphone")))beginCommand();}
 private void beginCommand(){if(!live()||!ready)return;lastError="";phase("Opening speech recognition");wake.pause(()->{if(!live()||!phase.equals("Opening speech recognition"))return;
  ...
  cancelSpeech();speech=SpeechRecognizer.createSpeechRecognizer(this);speech.setRecognitionListener(new RecognitionListener(){
   public void onReadyForSpeech(Bundle b){if(!live())return;phase("Listening");ToneGenerator tone=new ToneGenerator(AudioManager.STREAM_MUSIC,35);tone.startTone(ToneGenerator.TONE_PROP_BEEP,100);main.postDelayed(tone::release,250);}
   ...
   public void onError(int e){if(!ended&&(phase.equals("Listening")||phase.equals("Opening speech recognition"))){lastError=speechError(e);cancelSpeech();idle();}}
 ...
 static String speechError(int error){switch(error){
  case SpeechRecognizer.ERROR_NETWORK:case SpeechRecognizer.ERROR_NETWORK_TIMEOUT:return "Wake word heard, but the phone speech service needs an internet connection.";
  case SpeechRecognizer.ERROR_INSUFFICIENT_PERMISSIONS:return "Allow microphone access for Kutty and your phone speech service.";
  case SpeechRecognizer.ERROR_AUDIO:return "Your phone speech service could not open the microphone. Close other recording apps and try Ask now.";
  case SpeechRecognizer.ERROR_NO_MATCH:case SpeechRecognizer.ERROR_SPEECH_TIMEOUT:return "No question was heard. Say Hey Kutty, wait for the beep, then speak.";
  default:return "Phone speech recognition could not start (code "+error+"). Try Ask now or check Android speech settings.";
 }}
 private void idle(){if(!live())return;main.removeCallbacks(answerTimeout);requestId="";phase(wake.capturing?"Say Hey Kutty":"Starting microphone");wake.start();}

...
export function RecipeEditor({recipe,onClose,onSave,busy}:{recipe:Recipe|null;onClose:()=>void;onSave:(r:Recipe)=>Promise<boolean>;busy:boolean}){
 const [r,setR]=useState<Recipe>(()=>recipe?structuredClone(recipe):{id:'dish-'+crypto.randomUUID(),name:'',tamil:'',slot:'Breakfast',time:20,ingredients:[{name:'',amount:1,unit:'tbsp'}],steps:[''],allergens:[],note:'',source:'',prep:''});
 ...
 return <Dialog open onOpenChange={o=>!o&&!busy&&onClose()}><DialogContent className="recipe-dialog"><DialogTitle>{recipe?'Tweak this dish':'Add a family dish'}</DialogTitle><DialogDescription>Your saved version is used in meal plans and shopping lists. Family additions are not independently reviewed.</DialogDescription><form className="recipe-editor" onSubmit={async e=>{e.preventDefault();const parsed=recipeSchema.safeParse(r);if(!parsed.success){setError(parsed.error.issues[0].message);return}if(!checked){setError('Please review ingredients, allergens and texture before saving.');return}setError('');if(await onSave(parsed.data))onClose();else setError("Could not save this dish. Close this editor to see the planner’s error, or retry.")}}>

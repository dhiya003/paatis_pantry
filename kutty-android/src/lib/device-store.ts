...
const schema=z.object({version:z.literal(1),onboarded:z.boolean(),profile:z.object({name:z.string().trim().min(1).max(60),dob:date,anchor:date,allergies:z.array(z.enum(allergens))}),rules:z.record(z.object({kind:z.enum(['rotation','weekly','twiceWeekly','fortnightly','monthly','manual']),startsOn:date})),overrides:z.record(z.string()),kitchen:z.object({pantry:z.array(z.string()),favourites:z.array(z.string()),locks:z.array(z.string()),prepDone:z.array(z.string())}),changes:z.record(recipeSchema),removedIds:z.array(z.string())});
export type Saved=z.infer<typeof schema>;
const blank=():Saved=>({version:1,onboarded:false,profile:{...defaultProfile,allergies:[],anchor:todayIST()},rules:{},overrides:{},kitchen:structuredClone(emptyKitchen),changes:{},removedIds:[]});
...
function expose(s:Saved){const all={...s,removedIds:[]};return {profile:s.profile,rules:s.rules,overrides:s.overrides,kitchen:s.kitchen,recipes:catalogFor(s),removed:catalogFor(all).filter(r=>s.removedIds.includes(r.id)),onboarded:s.onboarded}}

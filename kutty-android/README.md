...
Features: English UI, side menu, editable traditional Tamil recipe adaptations with sources, age guidance, round-robin/frequency scheduling, overrides and locks, pantry, shopping, cooking mode, foreground recipe read-aloud, JSON backup/import, 6 PM Asia/Kolkata local notifications including the next day's menu, ingredients and preparation.
...
npm test
npm run build
rm -rf native/app/src/main/assets/wake
...
## Battery update (1.4.0)
The APK has no microphone, foreground-service or wake-lock permission, no voice service, no OpenAI connection and no bundled wake-word model. Upgrading clears obsolete voice notifications, conversation preferences and the encrypted API connection, without changing meal data or reminder preferences. The 6 PM alarm remains a scheduled local notification, not an always-running voice process.

CI checks compact mobile layout, meal and recipe navigation, native storage and back navigation. An instrumentation test inspects the installed package to confirm that voice-service permissions, registered services and wake-model assets are absent. Actual battery savings depend on the device and have not been measured on the user's phone.
